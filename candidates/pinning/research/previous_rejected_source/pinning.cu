...
/* Keep homogeneous coordinates (X/Z, Y/Z) through recovery. Normalizing
 * u1*G here would pay an inverse just before two projective point additions.
 * Both recovered keys are normalized together by the existing batch inverse. */
...
/* Preserve affine output for the single-point diagnostic kernel. */
