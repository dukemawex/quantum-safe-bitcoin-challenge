...
// Each table entry is H[hi]+L[lo], where m=2*index+1=hi*256+lo.
#pragma once
__host__ __device__ __forceinline__ void mixed_decode_entry(uint32_t t,int *ch,uint32_t *hi,uint32_t *lo){
    *ch=t<(1u<<17)?0:1+(int)((t-(1u<<17))>>16);
    uint32_t m=2*(t-compact_offset(*ch))+1;
    *hi=m>>8;*lo=m&255u;
}
__device__ __forceinline__ void mixed_finish_affine(uint64_t *x,uint64_t *y,const uint64_t *H,const uint64_t *L,uint64_t *inv){
    uint64_t slope[4];
...
__global__ void __launch_bounds__(256,2) mixed_table_prepare(
    ...
    if(active){
        uint32_t t=start+(uint32_t)i,hi,lo;int ch;mixed_decode_entry(t,&ch,&hi,&lo);
        if(hi){
            const uint64_t *hp=H+((size_t)ch*COMPACT_HI+hi)*8;
            const uint64_t *lp=L+((size_t)ch*COMPACT_LO+lo)*8;
...
__global__ void __launch_bounds__(256,2) mixed_table_finish(
    const uint64_t *L,const uint64_t *H,uint8_t *table,uint32_t start,int count,
    ...
    if(!active)return;
    uint32_t hi,lo;int ch;mixed_decode_entry(t,&ch,&hi,&lo);
    const uint64_t *lp=L+((size_t)ch*COMPACT_LO+lo)*8;
    uint64_t x[4],y[4];
    if(hi){
        const uint64_t *hp=H+((size_t)ch*COMPACT_HI+hi)*8;
        mixed_finish_affine(x,y,hp,lp,inv);
