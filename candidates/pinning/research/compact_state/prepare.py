...
finish=finish[:a]+'''    uint64_t delta[4],yb[4],m[4],t[4],s[4];
    // Saved U=Y*B, V=B^2 and inv=1/(W*V). Then delta=W/V,
    // inverse W=V*inv, and slope numerators are yR*V +/- U.
    _ModMult(yb,yR,ZZZ);
    _ModSqr(delta,W);
    _ModMult(delta,inv);        // W/V = xR-xP
    _ModMult(ZZZ,inv);          // 1/W
    _ModAdd256(W,xR,xR);
...
s=s[:a]+'''    // Save U=Y*B, W and V=B^2. The inverse tree consumes D=W*V.
    Load256(qzz,prod);
...
provenance={'base_submission':receipt['submission_id'],'base_fingerprint':receipt['fingerprint'],
 ...
 'production_modified':False,'change':'Search only: three fields U=Y*B,W,V=B^2; batch inverse D=W*V; rebuild128 search tree nodes, checkpoint126 upper nodes. Outer root hierarchy, table builder and fused path unchanged.'}
