...
__device__ __forceinline__ void qsb_block_inverse_rebuild(uint64_t *value,const uint64_t *roots) {
    __shared__ uint64_t products[4][512];
    ...
    int offset=0;
    #pragma unroll 1
    for(int count=256;count>2;count>>=1){
        int half=count>>1;
        if(tid<half){
            uint64_t a[5],b[5],out[5];
            #pragma unroll
