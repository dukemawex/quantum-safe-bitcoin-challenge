...
#define QSB_NEG_Y_MAC 1 /* under test: negated-deferred-ordinate MAC chain */
...
/* QSB_NEG_Y_MAC host shims: qsb_muladd_seed(r,a,b,c) = a*b + c mod p (the
 * seed is the already-negated deferred ordinate), qsb_negate_residue = -r. */
void qsb_muladd_seed(uint64_t *r,const uint64_t *a,const uint64_t *b,const uint64_t *c){
    BIGNUM *A=rd(a),*B=rd(b),*C=rd(c),*R=BN_new();
    fmul(R,A,B); fadd(R,R,C); wr(r,R);
    BN_free(A);BN_free(B);BN_free(C);BN_free(R);
}
void qsb_negate_residue(uint64_t *r){
    BIGNUM *A=rd(r); BIGNUM *Z=BN_new();
    fsub(A,Z,A); wr(r,A);   /* -r mod p */
    BN_free(A);BN_free(Z);
}
...
int main(int argc,char**argv){
    ...
    for(int t=0;t<cases;t++){
        ...
        BIGNUM *ny=BN_new();
#if QSB_NEG_Y_MAC
        /* scalar emits (x, -y) by design; the packed finish swaps slopes. */
        fsub(ny,PRIME,ay); if(BN_is_zero(ay))BN_copy(ny,ay);
#else
        BN_copy(ny,ay);
#endif
        if(BN_cmp(gx,ax)||BN_cmp(gy,ny)){ fails++;
