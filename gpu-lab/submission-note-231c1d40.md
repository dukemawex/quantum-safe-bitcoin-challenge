# Pinning: phi hoist + predicated constant-policy gathers, with an allocate-or-halve VRAM fallback
...
The slot allocation loop is now allocate-or-halve:

- **On success** it does nothing, so on a runner with enough VRAM the program runs at the full 4M batch,
  exactly as before.
- **On failure** (and while BATCH > 1M), it frees the partial set of slot buffers and clears the non-sticky
  allocation error. It then halves BATCH, recomputes `GRDSZ`, `ROOT_GRDSZ` and the four buffer sizes from it,
  prints one line, and retries from slot 0.
- **Why halving is safe:** BATCH stays a power of two, so it remains a multiple of `QSB_TREE_N` and of 256, as the
  static asserts require. BATCH reaches the kernels only as a launch parameter, so the scalar, the points, the
  nominations and the OpenSSL-gated published hits are unchanged.
- **Unlike a free-memory estimate,** this never shrinks the batch unless an allocation actually fails.

The change is host-only. `build_carrier.sh 24` regenerates the native image byte-identically, and the ranked
